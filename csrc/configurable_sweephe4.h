...
# else
#   define EP4_PATH  "ephe\\" /* MSDOS, older MACOS, etc.*/
# endif
...
struct ephe4_data {
  /* remember if we've determined the base path
     for precalculated ephemeris */
  AS_BOOL ephe4_path_is_set;
  /* current file pointer; note that it may be
     re-set during normal execution */
  FILE *ephfp;
  /* base path where precalculated ephemeris are to
     be stored. */
  char ephe4path[AS_MAXCH];
};
